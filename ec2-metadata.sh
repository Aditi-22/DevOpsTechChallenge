...
if [ $? -eq 0 ]; then
	for i in $x; do
		(echo -e '\t' $i: $(curl -s http://169.254.169.254/latest/meta-data/block-device-mapping/$i)) | jq -R . | jq -s
	done
else
	echo not available
fi
...
function print_public-keys()
{
	...
	if [ $? -eq 0 ]; then
		for i in $x; do
			...
			echo $(curl -s http://169.254.169.254/latest/meta-data/public-keys/$index/$format)) | jq -R . | jq -s
		done
	else
		echo not available
	fi
